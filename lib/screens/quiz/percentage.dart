...
class _PercentageState extends State<Percentage> {
  ...
  int percentLowerLimit = 1;
  int percentUpperLimit = 100;
  int numLowerLimit = 1;
    ...
    timer = Timer.periodic(const Duration(milliseconds: 500), (timer) {
      setState(() {
        ...
        if (getQuestion && showResult == false && timeLimit > 0) {
          percentage = (percentLowerLimit +
                  random.nextInt(percentUpperLimit - percentLowerLimit))
              .toString();

          num = (numLowerLimit + random.nextInt(numUpperLimit - numLowerLimit))
              .toString();
          correctAns = ((int.parse(percentage) / 100) * int.parse(num))
              .toStringAsFixed(2);

          displayQuestion = '$percentage % of $num';
        ...
        if (timeLeft <= 0 || showResult) {
          ...
          if (resultShown == false) {
            int points = calculatePoints(numUpperLimit, percentUpperLimit,
                numRows, correctSums, totalTime);
            String digits =
                "${(numUpperLimit - 1).toString().length} x ${(percentUpperLimit - 1).toString().length}";
            timeUpDialog(numRows, correctSums, incorrectSums, remainingTime, 0,
                points, digits);
          }
          timer.cancel();
        }
  ...
  showAgeRestrictionDialog(int rows, int marks, int incorrect, String timeTaken,
      int accuracy, String dateTime, int point, String username) {
  ...
  int calculatePoints(int digitsHigh, int percentHigh, int totalQuestions,
      int correctSums, int timeTaken) {
    int points = 0;

    points += ((digitsHigh - 1).toString().length * 10) +
        ((percentHigh - 1).toString().length * 10) * correctSums;
    if (correctSums > 0) {
      int sumPerSecs = timeTaken ~/ correctSums;
      points += ((1 / sumPerSecs) * 100).toInt();
    }
    if (totalQuestions == correctSums && totalQuestions >= 5) {
      points += ((totalQuestions / 3) * 8).toInt();
    }
    //print('$digits,$rows,$rowsCompleted,$timeTaken,$points,$reverse');
    return points;
  }

  timeUpDialog(int rows, int marks, int incorrect, String time, int accuracy,
      int point, String digits) async {
    String currentPhoneDate =
        DateFormat('EEE d MMMy, hh:mm:ss a').format(DateTime.now());
    String username = "Guest";
    if (userLoggedIn) {
      username = await send_result_to_firebase(
          "Percentage", "$digits", rows, marks, point, time, rows);
    }
                ...
                children: [
                  ...
                  TextButton(
                    onPressed: () {
                      ...
                      showAgeRestrictionDialog(rows, marks, incorrect, time,
                          accuracy, currentPhoneDate, point, username);
                    },
  ...
  Widget build(BuildContext context) {
    ...
    percentLowerLimit = int.parse(arguments['percentNumLower']);
    percentUpperLimit = int.parse(arguments['percentNumHigher']);
    numLowerLimit = int.parse(arguments['numLower']);
    numUpperLimit = int.parse(arguments['numUpper']);
